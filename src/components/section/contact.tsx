    ...
    return (
      <div className="py-20 bg-[#151515]">
        <div className="flex flex-col items-start justify-start gap-y-5 w-11/12 md:w-[85%] mx-auto my-0 px-3 transition-all duration-500 ease-in-out">
          ...
          <div className="font-medium font-untitled bg-gradient-to-b from-slate-600 to-slate-300 bg-clip-text text-transparent">
            ...
            <div className={`flex flex-row items-center flex-wrap  gap-x-3`}>
              {contactText
                ...
                ?.map((arrayText: Array<string>, index: number) => (
                  <ParallaxCharacter
                    key={index}
                    subContainerClassName={`text-6xl flex flex-row items-center flex-wrap`}
                    className={`flex flex-row items-center flex-wrap !my-1 text-4xl md:text-6xl text-white text-transparent font-untitled`}
                    textArray={arrayText}
                  />
                ))}
            </div>
            <div
              className={`flex flex-row gap-x-3 items-center flex-wrap`}
            >
              {contactText
                ?.slice(4)
                ?.map((arrayText: Array<string>, index: number) => (
